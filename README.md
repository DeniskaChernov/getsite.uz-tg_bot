...
Сотрудников по имени не называем: клиенту «менеджер» и контакты getsite.
Запрос «удалите мои данные» удаляет историю сразу; /health проверяет webhook и БД.
...
CRM-дубль лидов, A/B приветствий, UZ-вычитка носителем.
Основной план бота закрыт: регистрация, ИИ-диалог, подтверждение, лиды, follow-up с кнопками, референсы, воронка в `/stats`, свежая заявка при повторном `/start`.
